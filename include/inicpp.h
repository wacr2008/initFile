...
namespace ini
{
    ...
    public:
        ...
        void decode(std::istream &is)
        {
            char delimLinux='\r';
            char delimWindows='\n';
            char delim=delimWindows;
            bool checkDelim=true;
            clear();
            int lineNo = 0;
            ...
            while(!is.eof() && !is.fail())
            {
                ...
                if(checkDelim){
                    std::getline(is, line, delimWindows);
                    if(line.length()>1 && line[line.length()-1]==delimLinux){
                        delim = delimLinux;
                ...
                if(line.length()>0 &&(line[0]==delimLinux||line[0]==delimWindows)){
